...
"""
RAT UNIFIED GUI v3.2 (Graphical User Interface)
--------------------------------------------------------------------------------
...
class RATUnifiedGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("RAT Unified GUI v3.2")
        ...
        self.vars = {
            ...
            "v_smooth_factor": tk.StringVar(value="1000"),
            ...
            "fac_1": tk.BooleanVar(value=True),   # 1 = One-way (Default ON)
            "fac_2": tk.BooleanVar(value=True),   # 2 = Two-way (Default ON)
            "fac_4": tk.BooleanVar(value=False),  # 4 = Ramp
            "fac_5": tk.BooleanVar(value=False),  # 5 = Non-mainline
            "fac_6": tk.BooleanVar(value=False),  # 6 = Non-inventory
            "fac_7": tk.BooleanVar(value=False),  # 7 = Planned/Unbuilt
        ...
        self.advanced_defaults = {
            ...
            "TREND_WINDOW_FT": tk.StringVar(value="2000"), # was 1000
            "DIP_THRESHOLD_FT": tk.StringVar(value="6.5"),
            "BRIDGE_MAX_LEN_FT": tk.StringVar(value="8200"),
            # Per-functional-system smoothing factors.
            # These are the national calibration defaults from the RAT National
            # Calibration Engine v3.8 full 50-state sweep. State-specific values
            # from national_smoothing_factors.json will override these at run time.
            "H_SMOOTH_FACTOR_FS2": tk.IntVar(value=200),
            "V_SMOOTH_FACTOR_FS2": tk.IntVar(value=1000),
            "H_SMOOTH_FACTOR_FS3": tk.IntVar(value=200),
            "V_SMOOTH_FACTOR_FS3": tk.IntVar(value=1200),
            "H_SMOOTH_FACTOR_FS4": tk.IntVar(value=200),
            "V_SMOOTH_FACTOR_FS4": tk.IntVar(value=400),
            "H_SMOOTH_FACTOR_FS5": tk.IntVar(value=200),
            "V_SMOOTH_FACTOR_FS5": tk.IntVar(value=600),
            "H_SMOOTH_FACTOR_FS6": tk.IntVar(value=200),
            "V_SMOOTH_FACTOR_FS6": tk.IntVar(value=400),
            "H_SMOOTH_FACTOR_FS7": tk.IntVar(value=200),
            "V_SMOOTH_FACTOR_FS7": tk.IntVar(value=400),
        }
    ...
    def _build_ui(self):
        ...
        lf_fac.grid(row=5, column=0, columnspan=5, sticky="ew", pady=8)
    ...
    def _collect_params_json(self, out_dir):
        ...
        # Helper to safely convert float fields without crashing on "none" or empty strings
        ...
        # Intercept "None" or "none" before sending it to float()
        ...
        for k, var in self.advanced_defaults.items():
            ...
            try:
                if val.lower() == "none" or not val:
                    continue # Leaves data[k] alone if missing/none
        ...
        # =========================================================
        # INJECT SMART CALIBRATION FACTORS BEFORE SAVING
        # =========================================================
                ...
                if state_fips_input in master_dict:
                    ...
                    for key, custom_val in custom_factors.items():
                        if custom_val is not None:
                            data[key] = custom_val 
                    logging.info(f"Loaded state-specific smoothing factors for FIPS {state_fips_input} from national_smoothing_factors.json.")
            except Exception as e:
                logging.error(f"Could not apply smoothing factors from national_smoothing_factors.json: {e}")
        # =========================================================
    ...
    def _load_config(self):
        ...
        try:
            ...
            for k, v in data.items():
                # Prevent null values from displaying as the string "None" in UI boxes
                if v is None:
                    v = ""
    ...
    def _run_validator(self, out_dir):
        ...
        # Use os.walk for bulletproof OS-agnostic subfolder searching
        h_files = []
        v_files = []
                ...
                while True:
                    ...
                    try:
                        ...
                        if attempt < max_attempts:
                            wait_s = min(60, 2 ** attempt)
                            logging.warning(
                                f"Socrata returned malformed JSON at offset {offset}. "
                                f"Attempt {attempt}/{max_attempts}. Retrying in {wait_s}s..."
                            )
                            time.sleep(wait_s)
                            continue
                        raise RuntimeError(
                            f"Socrata returned malformed JSON at offset {offset} after "
                            f"{max_attempts} attempts: {parse_err}"
                        )
