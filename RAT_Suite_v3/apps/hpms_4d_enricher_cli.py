...
"""
RAT 4D ENRICHER CLI v3.0 (Z & M Coordinate Generator)
--------------------------------------------------------------------------------
...
def main():
    ...
    out_df = out_df.drop(columns=["WKT"], errors="ignore") # Drop once
    
    out_df.to_csv(csv_out, index=False)
    gdf = gpd.GeoDataFrame(out_df, geometry="geometry_3d", crs="EPSG:4326")
    ...
    try:
        ...
        cols_to_drop = [c for c in gdf_blender.columns if c.upper() == "WKT_ZM"]
