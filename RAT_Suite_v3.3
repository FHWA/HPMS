# Roadway Alignment Tool (RAT) Suite v3.3
...
Version 3.3 introduces a nationally calibrated smoothing parameter system derived from a full 50-state HPMS data sweep, replacing the manually estimated defaults used in prior versions. The suite continues to use a modular, decoupled architecture in which the Unified GUI orchestrates isolated CLI scripts to prevent memory issues during statewide batch processing.
...
## Documentation
For methodology, mathematical formulas, and parameter reference, see the **Technical Manual** (`docs/RAT_Suite_v3_3_Technical_Manual.md`). The `core/calibration_audit.csv` file documents the confidence scores and selection rationale for each state and functional system in the national smoothing parameter dictionary.
...
*RAT Suite v3.3 — Federal Highway Administration, Office of Highway Policy Information*
***
