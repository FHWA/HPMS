...
"""
RAT PLAN & PROFILE PDF RENDERER v3.2 (Matplotlib Generator)
--------------------------------------------------------------------------------
...
CHANGES FROM:
  - Fixed UnboundLocalError crash risk for page_mp_span.
  - Fixed X-Axis scaling logic to properly size pages on massive routes.
  - Increased USGS Basemap timeout to 30 seconds to prevent silent failures.
...
def fetch_basemap_image(min_lon, min_lat, max_lon, max_lat, width=1200, height=600):
    ...
    try:
        # Increased timeout to 30s to prevent silent failures from slow USGS servers
        r = requests.get(BASEMAP_URL, params=params, timeout=30)
...
def make_pdf(vertices_csv, horizontal_csv, vertical_csv, pdf_out, route_id):
    ...
    os.makedirs(os.path.dirname(new_pdf_out) or ".", exist_ok=True)
    
    with PdfPages(new_pdf_out) as pdf:
        for p in range(num_pages):
            ...
            center_lon = np.mean(sub_lon)
            center_lat = np.mean(sub_lat)
            box_radius = 0.005  
            
            b_min_lon, b_max_lon = center_lon - box_radius, center_lon + box_radius
            b_min_lat, b_max_lat = center_lat - box_radius, center_lat + box_radius
            
            bg_img = fetch_basemap_image(b_min_lon, b_min_lat, b_max_lon, b_max_lat, width=600, height=600)
            ...
            fig.text(0.85, 0.05, f"SHEET {p+1} OF {num_pages}", fontsize=14, fontweight='bold')
            fig.text(0.85, 0.035, f"DATE: {date_str}", fontsize=10)
            ...
            if not hc.empty:
                for _, c in hc.iterrows():
                    ...
                    if c_mask.any():
                        ...
                        base_color = colors_h.get(c['Dir'], 'green')
                        is_spiral = c.get('Transition_Type', 'Simple') == 'Spiral'
                        
                        # Plotting Logic: Central Curve vs Spiral Tails
                        if is_spiral and 'SC_Dist' in c and pd.notna(c['SC_Dist']):
                            sc_ft = c['SC_Dist'] * FEET_PER_METER
                            cs_ft = c['CS_Dist'] * FEET_PER_METER
                            
                            # Entering Spiral Tail (Dashed)
                            ts_sc_mask = (sub_dist >= c_start_ft) & (sub_dist < sc_ft)
                            if ts_sc_mask.any(): ax_plan.plot(rot_x[ts_sc_mask], rot_y[ts_sc_mask], color=base_color, linewidth=5, alpha=0.6, linestyle='--')
                            # Central Circular Curve (Solid Thick)
                            sc_cs_mask = (sub_dist >= sc_ft) & (sub_dist <= cs_ft)
                            if sc_cs_mask.any(): ax_plan.plot(rot_x[sc_cs_mask], rot_y[sc_cs_mask], color=base_color, linewidth=6, alpha=0.6)
                            # Exiting Spiral Tail (Dashed)
                            cs_st_mask = (sub_dist > cs_ft) & (sub_dist <= c_end_ft)
                            if cs_st_mask.any(): ax_plan.plot(rot_x[cs_st_mask], rot_y[cs_st_mask], color=base_color, linewidth=5, alpha=0.6, linestyle='--')
                        else:
                            # Standard Simple Curve (Solid Thick)
                            ax_plan.plot(c_x, c_y, color=base_color, linewidth=6, alpha=0.5, zorder=3)
                        ...
                        # Annotation Logic
                        if is_spiral and 'Calibrated_SC_MP' in c and pd.notna(c['Calibrated_SC_MP']):
                            annot = f"[SPIRAL] Class {c.get('Bin', 'A')}\nMin R={min_r_ft:.0f}' Δ={c['Delta']:.1f}°\nTS: RP {c.get('Calibrated_Start_MP', 0):.3f}\nSC: RP {c.get('Calibrated_SC_MP', 0):.3f}\nCS: RP {c.get('Calibrated_CS_MP', 0):.3f}\nST: RP {c.get('Calibrated_End_MP', 0):.3f}"
                        else:
            ...
            # FIXED UNBOUND LOCAL ERROR & PROPER SCALING
            page_mp_span = mp_max - mp_min
            page_dist = sub_dist[-1] - sub_dist[0]
            if page_dist > 0:
                page_mp_span = (PAGE_LENGTH_FT / page_dist) * (mp_max - mp_min)
            ...
            if bg_img:
                ax_inset = fig.add_axes([0.85, 0.80, 0.1, 0.15], anchor='NE', zorder=1)
                ax_inset.imshow(bg_img, extent=[b_min_lon, b_max_lon, b_min_lat, b_max_lat])
                ax_inset.axis('off')
            ...
            # SIGHT DISTANCE SAFETY BAND
            if 'Available_Sight_Dist_Ft' in vtx.columns and 'V_Curve_Type' in vtx.columns:
                sub_asd = vtx.loc[mask, 'Available_Sight_Dist_Ft'].to_numpy()
                sub_v_type = vtx.loc[mask, 'V_Curve_Type'].to_numpy()
                y_min_limit = avg_elev - 95 # Bottom of the Y-axis
                ax_prof.plot(sub_mp, [y_min_limit] * len(sub_mp), color='gray', linewidth=4, alpha=0.3, zorder=0)
                
                # MASK: Must be <400ft AND NOT located inside a SAG curve
                blind_mask_local = (sub_asd < 400) & (sub_v_type != 'SAG')
                if blind_mask_local.any():
                    ax_prof.scatter(sub_mp[blind_mask_local], [y_min_limit] * np.sum(blind_mask_local), 
                                    color='red', s=10, marker='s', label='Sight Warning (<400ft)', zorder=4)
            # INTERCHANGE TOPOLOGY MARKERS
            if 'Topology_Node' in vtx.columns:
                topo_subset = vtx.loc[mask & (vtx['Topology_Node'] != 'None')].copy()
                if not topo_subset.empty:
                    unique_topo = topo_subset.drop_duplicates(subset=['Intersecting_Route'])
                    for _, row in unique_topo.iterrows():
                        l_color = 'orange' if row['Topology_Node'] == 'Grade-Separated' else 'green'
                        ax_prof.axvline(x=row['Milepost'], color=l_color, linestyle='--', linewidth=1.5, alpha=0.7, zorder=5)
                        ax_prof.text(row['Milepost'], avg_elev + 85, f"{row['Topology_Node']}\n{row['Intersecting_Route']}", 
                                    color=l_color, fontsize=7, fontweight='bold', ha='center', va='top', rotation=90,
                                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec=l_color, alpha=0.8))
            ...
            ax_plan.ticklabel_format(useOffset=False, style='plain', axis='x')
            ax_prof.ticklabel_format(useOffset=False, style='plain', axis='x')
